...
== Restassured SE module

There is a newer module, where we can use the configured Restassured without a CDI/Weld container.
This method can be faster and can be applied in multithreaded environments, i.e. in parallel tests.

To use this module we should add this dependency to the pom.xml instead of `roaster-restassured-cdi`:
...
<dependency>
    <groupId>hu.icellmobilsoft.roaster</groupId>
    <artifactId>roaster-restassured-se</artifactId>
</dependency>
----
