...
The `hu.icellmobilsoft.roaster.hibernate.common.helper.TransactionHelper` class can help with transaction management, and automatic rollback on exception.
Example:

[source,java]
----
TransactionHelper.runInTransaction(entityManager, () -> entityManager.persist(new ExampleEntity()));
----

== EntityContainer

`hu.icellmobilsoft.roaster.hibernate.common.helper.EntityContainer` helps with automatic deletion of the newly created entities which are created in the test method.
The entities saved through this class are deleted in reverse order at the `deleteAll` or `close` method call.
...
1. With `@AutoClose` annotation the created entities will be automatically deleted at the end of the test run.
