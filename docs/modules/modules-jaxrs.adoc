...
<dependency>
    ...
    <artifactId>roaster-jaxrs-cdi</artifactId>
</dependency>
----
...
The abstract class `hu.icellmobilsoft.roaster.jaxrs.se.response.ResponseProcessor` was created to assist in testing a specific endpoint.
...
[source,java]
----
import hu.icellmobilsoft.roaster.jaxrs.response.producer.spi.ConfigurableResponseProcessor;

@Dependent
public class CustomConfigurableResponseProcessor<RESPONSE> extends ConfigurableResponseProcessor<RESPONSE> {
...
There is a newer module, where we can use the configured Jaxrs without a CDI/Weld container.
This method can be faster and can be applied in multithreaded environments, i.e. in parallel tests.

To use this module we should add this dependency to the pom.xml instead of `roaster-jaxrs-cdi`:

[source,java]
----
import hu.icellmobilsoft.roaster.jaxrs.se.response.ConfigurableResponseProcessor;
import hu.icellmobilsoft.roaster.jaxrs.se.response.ConfigurableResponseProcessorFactory;

class ExampleTest {
    private ConfigurableResponseProcessor<ExampleResponse> responseProcessor = ConfigurableResponseProcessorFactory
            .create("testsuite.rest.exampleService.example");
    private ConfigurableResponseProcessor<ExampleResponse> badRequestResponseProcessor = ConfigurableResponseProcessorFactory
            .create("testsuite.rest.exampleService.example", HttpStatus.SC_BAD_REQUEST);
}
----
...
<dependency>
    <groupId>hu.icellmobilsoft.roaster</groupId>
    <artifactId>roaster-jaxrs-se</artifactId>
</dependency>
----
